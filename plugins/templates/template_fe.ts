...
export const generateFacturaComercial = (
  doc: jsPDF,
  dte: SVFC_FC_Firmado,
  QR: Uint8Array,
  ...
  const giroSplit = doc.splitTextToSize(
    `GIRO : ${dte.receptor.descActividad ? dte.receptor.descActividad : "-"}`,
    120
  );
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const docName = doc.splitTextToSize("COMPROBANTE DE FACTURA COMERCIAL", 30);
    doc.setFontSize(6);
    returnBoldText(
      doc,
      docName,
...
export const footerDocument = (
  ...
  doc.text("Total Otros montos no afectos: ", 127, rectMargin + 33);
  doc.text("Total a Pagar: ", 127, rectMargin + 36);
